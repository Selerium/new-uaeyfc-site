...
export default async function SpecificProgram({ params }: {
    ...
    return (
      <div className="w-full h-fit">
        {disable && <div className="flex flex-col justify-center items-center gap-4 w-full h-full fixed top-0 left-0 bg-neutral-100 text-black z-50">
          <h1 className="text-4xl font-semibold">{'THIS PAGE IS UNDER CONSTRUCTION :)'}</h1>
          <p>{'(sorry about that)'}</p>
          <Link href='/' className="button">GO TO HOME</Link>
        </div>}
